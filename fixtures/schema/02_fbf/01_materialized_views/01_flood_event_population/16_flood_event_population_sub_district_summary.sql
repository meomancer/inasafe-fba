DROP MATERIALIZED VIEW IF EXISTS public.mv_hazard_event_population_sub_district_summary;
CREATE MATERIALIZED VIEW public.mv_hazard_event_population_sub_district_summary AS
    WITH non_hazarded_count as (
        ...
        where a.district_id is not null and a.sub_district_id is not null and a.village_id is not null
        group by a.hazard_event_id, a.district_id, a.sub_district_id
        )
    select
    ...
    from
         hazarded_count
         LEFT JOIN sub_district_trigger_status trigger_status on hazarded_count.hazard_event_id = trigger_status.hazard_event_id and hazarded_count.sub_district_id = trigger_status.sub_district_id
         JOIN non_hazarded_count on hazarded_count.sub_district_id = non_hazarded_count.sub_district_id
         JOIN sub_district region on hazarded_count.sub_district_id = region.sub_dc_code
  WITH NO DATA;
